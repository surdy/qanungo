...
/// The manifest's `capture.source_metadata`, carrying the capturing machine's UTC offset when the
/// snapshot recorded one — the fact the heatmap reads. Empty otherwise, which is what a capture from
/// before the metadata existed serves, and what every other fixture here stays in.
fn source_metadata(session: &ArchivedSession) -> String {
    match &session.utc_offset {
        Some(offset) => format!(r#"{{"utc_offset":"{offset}"}}"#),
        None => "{}".to_owned(),
    }
}
