...
fn the_page_renders_the_fleet_panel_and_computes_nothing() {
    ...
    // Every invariant the page already held, restated over the grown file: the only link the
    // page writes is the catalogue's (`ruleLink`), and the fleet panel adds none of its own.
    assert_no_link_but_the_catalogue(&body);
    assert!(
        !body.contains("innerHTML"),
        "every value is set as text, never parsed as markup",
